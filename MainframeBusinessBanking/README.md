...
* **COBOL batch** nightly account-opening job that also issues linked debit cards
* **DB2** relational model for applications, customers, signatories, accounts, cards, documents, and audit
...
│   └── BATCH/         - Batch account-opening and card-issuing program (BACBAT01.cbl)
├── COPYBOOK/          - COBOL copybooks for records, COMMAREA, cards, errors, common areas
├── DB2/
...
│   ├── OUTPUT/        - Runtime outputs (account and card reports)
...
It validates PEP/sanctions flags, document checklist, and risk rating before submission. Submitted records are inserted into `TB_BAC_APPLICATION`, `TB_BAC_SIGNATORY`, `TB_BAC_DOCUMENT`, and `TB_BAC_AUDIT`. A debit card can be requested during the application flow and is generated when the account is opened.

### Batch Account Opening

* **Program:** `BACBAT01`
* **Tables:** `TB_BAC_APPLICATION`, `TB_BAC_CUSTOMER`, `TB_BAC_ACCOUNT`, `TB_BAC_CARD`, `TB_BAC_AUDIT`

`BACBAT01` opens a cursor over submitted/approved applications, creates a customer and account for each, issues a linked debit card when requested, updates the application status to `OP`, writes an audit row, and produces account-opening and card-issue reports.
...
| `TB_BAC_CARD` | Issued debit / prepaid card master |
| `TB_BAC_DOCUMENT` | KYC document checklist |
| `TB_BAC_AUDIT` | Status change audit trail |

Sequences `BAC_APP_SEQ`, `BAC_SIG_SEQ`, `BAC_CUST_SEQ`, `BAC_ACC_SEQ`, and `BAC_CARD_SEQ` generate surrogate IDs.
...
3. `BACBAT01` - processes submitted/approved applications, creates customer and account records, and writes `OUTPUT/ACCOUNT_OPEN_RPT.txt` and `OUTPUT/CARD_ISSUE_RPT.txt`

### Expected Output

```text
APP CREATED: APP0000004 STATUS: SB CARD: DC DAILY LIMIT: 2,500.00 BUSINESS: Riverfront Tech Inc
...
1. Run `DB2/DDL/*.sql` to create tables, indexes, and sequences (including `CRTTBL07.sql` for `TB_BAC_CARD` and `CRTSEQ02.sql` for `BAC_CARD_SEQ`).
...
7. Submit `JCL/BACBATJ.jcl` to run the batch account-opening and card-issuing job.
